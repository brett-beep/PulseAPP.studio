 ...
 *   FINLIGHT_API_KEY=your_key node scripts/finlight-ticker-aapl.mjs
 *
 * Or export the key first:
 *   export FINLIGHT_API_KEY=your_key
 *   node scripts/finlight-ticker-aapl.mjs
 */

const FINLIGHT_API_BASE = "https://api.finlight.me";

function getDateFromHoursAgo(hours) {
...
async function main() {
  ...
  if (!apiKey || apiKey === "your_actual_key_here") {
    console.error("❌ Set FINLIGHT_API_KEY in the environment (your real key, not the placeholder).");
    process.exit(1);
  }
  ...
  articles.forEach((a, i) => {
    ...
    if (a.summary) console.log(`   Summary: ${String(a.summary).slice(0, 120)}...`);
    console.log("");
  });
