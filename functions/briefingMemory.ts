...
/**
 * Complete memory save workflow for a briefing.
 * Saves BriefingMemory, updates StoryTracker for all stories + watch items, records delivery.
 * MUST be awaited before returning HTTP response to guarantee persistence.
 */
export async function saveBriefingMemoryComplete(
  ...
  briefingDate: string,
  analyzedBrief: {
    macro_selections?: Array<{ story_key?: string; hook?: string; facts?: string[] }>;
    portfolio_selections?: Array<{ ticker?: string; story_key?: string; hook?: string; facts?: string[] }>;
    watch_items?: { primary?: { event?: string; date?: string; why_it_matters?: string }; secondary?: { event?: string; date?: string; why_it_matters?: string } };
  },
  tickerMarketMap: Record<string, { quote?: { current_price?: number | null; change_pct?: number } }>,
  scriptwriterResult?: { watch_items_mentioned?: Array<{ event?: string; date?: string }> }
): Promise<void> {
  console.log("💾 [Memory] Starting synchronous save (blocking)...");
  ...
  try {
    ...
    console.log("✅ [Memory] All memory saves completed successfully");
  } catch (memoryErr: any) {
    console.error("⚠️ [Memory] Failed:", memoryErr?.message, memoryErr?.stack ?? memoryErr);
    throw memoryErr; // Re-throw so caller knows it failed
  }
}